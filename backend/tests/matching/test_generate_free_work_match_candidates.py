...
def test_separation_identifiants(tmp_path):
    ...
    matching_dirs = list((tmp_path / "backend" / "data" / "processed" / "matching" / "free_work_vs_france_travail").iterdir())
...
def test_descriptions_absentes_and_progress_json(tmp_path):
    ...
    progress_file = tmp_path / "backend" / "data" / "processed" / "matching" / "progress.json"
...
def test_progress_write_robustness_success_after_retry(tmp_path):
    ...
    with patch("backend.scripts.generate_free_work_match_candidates.PROCESSED_DATA_ROOT", tmp_path / "backend" / "data" / "processed"), \
         patch("backend.scripts.generate_free_work_match_candidates.time.sleep") as mock_sleep, \
...
def test_progress_write_failed_does_not_mask_original_exception(tmp_path):
    ...
    with patch("sys.argv", test_args), \
         patch("backend.scripts.generate_free_work_match_candidates.PROCESSED_DATA_ROOT", tmp_path / "backend" / "data" / "processed"), \
         patch("backend.scripts.generate_free_work_match_candidates.generer_matching", side_effect=ValueError("Erreur métier")), \
         patch.object(Path, "replace", mock_replace), \
