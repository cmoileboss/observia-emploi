...
ROME_MODE_WITH = "WITH_ROME"
ROME_MODE_WITHOUT = "WITHOUT_ROME"
ROME_METHOD_INHERITED_FROM_FT_MATCH = "INHERITED_FROM_FT_MATCH"
ROME_METHOD_NOT_ASSIGNED = "NOT_ASSIGNED"
ROME_STATUS_INHERITED = "INHERITED"
ROME_STATUS_PENDING = "PENDING"
...
def main() -> None:
    ...
    try:
        ...
        rome_mode = ROME_MODE_WITH if args.rome_run_dir else ROME_MODE_WITHOUT
        rome_run_dir = Path(args.rome_run_dir) if args.rome_run_dir else None
        ...
        for path, label in [
            ...
            if not path.exists():
                raise FileNotFoundError(f"{label} introuvable : {path}")
        if rome_run_dir is not None and not rome_run_dir.exists():
            raise FileNotFoundError(f"rome-run-dir introuvable : {rome_run_dir}")
        ...
        # ROME counters
        rome_status_counts = {}
        rome_inherited_from_ft_count = 0
        rome_pending_count = 0
        invalid_inherited_rome_count = 0
        ...
        for sync_item in offers_to_process:
            ...
            if rome_mode == ROME_MODE_WITH:
                ...
                if triage_dec.get(
                    ...
                    rome_inherited_from_ft_count += 1
                else:
                    rome_code = None
                    rome_status = ROME_STATUS_PENDING
                    rome_method = ROME_METHOD_NOT_ASSIGNED
                    rome_pending_count += 1
            ...
            record = {
                ...
                "rome_score": rome_score,
                "rome_margin": rome_margin,
                "rome_candidates": rome_candidates,
            ...
            if rome_method == ROME_METHOD_INHERITED_FROM_FT_MATCH:
                ...
                if not proof.get("france_travail_id") or proof.get("rome_code") != rome_code:
                    invalid_inherited_rome_count += 1
        ...
        deactivation_output_ids = [str(record["source_id"]) for record in offers_to_deactivate]
        output_process_ids = set(process_output_ids)
        output_deactivation_ids = set(deactivation_output_ids)
        duplicate_process_output_ids = sorted(
        ...
        integrity_report.update({
            ...
            "rome_pending_count": rome_pending_count,
            "invalid_inherited_rome_count": invalid_inherited_rome_count,
            "artificial_rome_code_count": invalid_inherited_rome_count,
            "rome_integrity_valid": invalid_inherited_rome_count == 0,
        })
        integrity_report["integrity_valid"] = all([
            active_partition_valid,
            deactivation_partition_valid,
            count_equation_valid,
            active_equation_valid,
            len(missing_process_ids_in_triage) == 0,
            len(missing_process_ids_in_rome) == 0,
            len(unexpected_triage_ids_outside_snapshot) == 0,
            len(unexpected_rome_ids_outside_snapshot) == 0,
            len(process_unchanged_overlap) == 0,
            len(process_deactivation_overlap) == 0,
            len(unchanged_deactivation_overlap) == 0,
            len(missing_process_ids_in_outputs) == 0,
            len(unexpected_process_ids_in_outputs) == 0,
            len(duplicate_process_output_ids) == 0,
            len(missing_deactivation_ids_in_outputs) == 0,
            len(unexpected_deactivation_ids_in_outputs) == 0,
            len(duplicate_deactivation_output_ids) == 0,
            invalid_inherited_rome_count == 0,
        ])
