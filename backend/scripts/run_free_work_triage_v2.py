...
def validate_candidate_matches(rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for source_id, row in lookup.items():
        for key in ("free_work_title", "free_work_title_normalized", "top_candidates"):
            if key not in row:
                raise ValueError(
                    f"candidate_matches[{source_id}] : champ obligatoire absent : {key}")
...
def validate_france_travail(rows: list[dict[str, Any]]) -> None:
    ...
    for index, row in enumerate(rows):
        ...
        if str(france_travail_id) in seen:
            raise ValueError(
                f"france_travail contient un identifiant dupliqué : {france_travail_id}")
        seen.add(str(france_travail_id))
        for key in ("title", "description", "rome_code"):
            if key not in row:
                raise ValueError(
                    f"france_travail[{france_travail_id}] : champ obligatoire absent : {key}")
...
def build_triage_entry_from_match(match_entry: dict[str, Any]) -> dict[str, Any]:
    ...
    company = match_entry.get("free_work_company_normalized") or match_entry.get(
        "free_work_company") or ""
    ...
    data_coverage = 45 + (25 if description else 0) + \
        (10 if company else 0) + (15 if postal_code else 0)
    return {
...
def run_fresh_triage_v2(
    ...
    try:
        free_work_rows = require_list(
            load_json_file(
                free_work_input,
                "offers_normalized"),
            "offers_normalized")
        france_travail_rows = require_list(
            load_json_file(
                france_travail_input,
                "france_travail_snapshot"),
            "france_travail_snapshot")
        candidate_matches = require_list(
            load_json_file(
                candidate_matches_input,
                "candidate_matches"),
            "candidate_matches")
        ...
        if missing_in_free_work:
            raise ValueError(
                f"{len(missing_in_free_work)} candidate_matches sans offre normalisée, exemple : {missing_in_free_work[:5]}")  # pylint: disable=line-too-long
        if missing_in_candidates:
            raise ValueError(
                f"{len(missing_in_candidates)} offres normalisées sans candidate_matches, exemple : {missing_in_candidates[:5]}")  # pylint: disable=line-too-long
        ...
        for index, match_entry in enumerate(candidate_matches, start=1):
            ...
            if index % 1000 == 0 or index == len(candidate_matches):
                write_progress(output_dir, "PREPARING_TRIAGE_INPUT",
                               index, len(candidate_matches), start)
        ...
        for index, match_entry in enumerate(candidate_matches, start=1):
            ...
            expected_skills = free_work_details.get("skills") if isinstance(
            ...
            if index % 1000 == 0 or index == len(candidate_matches):
                ...
                print(
                    f"[V2 fresh] {index}/{len(candidate_matches)} - {index / len(candidate_matches) * 100:.2f}% - {speed:.1f} offres/s - ETA {eta:.1f}s")  # pylint: disable=line-too-long
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--free-work-input",
        required=True,
        help="Chemin vers offers_normalized.json.")
    parser.add_argument(
        "--france-travail-input",
        required=True,
        help="Chemin vers france_travail_offers_snapshot.json.")
    parser.add_argument("--candidate-matches-input", required=True,
                        help="Chemin vers candidate_matches.json.")
