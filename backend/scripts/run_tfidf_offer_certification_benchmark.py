"""Exécute le benchmark TF-IDF hors base et exporte ses résultats."""
...
OUTPUT_FILENAMES = (
    ...
    "tfidf_rankings.jsonl",
    "benchmark_summary.json",
)


def _read_json_object(path: Path, description: str) -> tuple[Mapping[str, Any], bytes]:
...
def run_tfidf_benchmark(
    ...
    """Charge les deux artefacts puis exécute TF-IDF sur le split demandé."""
...
def build_tfidf_benchmark_artifacts(
    run: BenchmarkRun,
    offers_sha256: str,
    catalogue_sha256: str,
) -> dict[str, bytes]:
    """Construit le manifeste, les classements JSONL et le résumé du run."""
    ...
    manifest = {
        ...
        "methode": {
            ...
            "score": "similarite_cosinus_tfidf_brute",
        ...
        "artefacts": {
            "classements": "tfidf_rankings.jsonl",
...
def export_tfidf_benchmark_artifacts(
    ...
    if existing_paths:
        raise FileExistsError(
            "Des artefacts TF-IDF existent déjà : "
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Analyse les chemins, le split et la taille du top K."""
    parser = argparse.ArgumentParser(
        description=(
            "Classe toutes les certifications RNCP actives par TF-IDF, sans base."
        )
    )
    parser.add_argument(
        "--offers",
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Dossier de sortie des artefacts TF-IDF.",
    )
    return parser.parse_args(argv)


def main() -> None:
    """Exécute TF-IDF, exporte les artefacts et affiche le résumé."""
    args = parse_args()
    try:
        ...
        export_tfidf_benchmark_artifacts(artifacts, args.output_dir)
    except Exception as exc:
        print(f"Erreur du benchmark TF-IDF : {exc}", file=sys.stderr)
        sys.exit(1)
