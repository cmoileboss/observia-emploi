...
def analyze_intermediaries(
    ...
    for match in candidate_matches:
        ...
        item = {
            ...
            "geography": (
                best.get("geography_comparison") or {}).get("result"),
            ...
            "simulated_reason": "STRONG_MATCH_VIA_RECRUITMENT_INTERMEDIARY" if strong_match_via_intermediary(  # pylint: disable=line-too-long
                best,
                role) else None,
        }
        ...
        if v2_by_id[source_id]["decision"] != "PRESENT_IN_FT_SNAPSHOT" and strong_match_via_intermediary(  # pylint: disable=line-too-long
                best,
                role):
    ...
    md_lines = [
        ...
        "",
        f"- Preuve explicite d'intermédiation: {payload['summary']['explicit_intermediary_detected']}",  # pylint: disable=line-too-long
        f"- Intermédiation possible mais non prouvée: {payload['summary']['possible_intermediary_not_proven']}",  # pylint: disable=line-too-long
        ...
        f"- Potentiellement reclassables PRESENT: {payload['summary']['potentially_reclassable_present']}",  # pylint: disable=line-too-long
        "",
