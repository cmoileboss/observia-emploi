...
def classify_triage(fw_item, best_cand, second_cand, cand_count, fw_tokens, fw_desc_norm, fw_company_norm, fw_pc, fw_dept, fw_locality_norm, fw_romes):
    ...
    if comp_comp["match_type"] in ["EXACT_NORMALIZED", "ALIAS_MATCH"] and geo_comp["result"] == "DIFFERENT":
...
def main():
    ...
    parser.add_argument("--free-work-input", required=True, help="offers_normalized.json de Free-Work.")
    ...
    parser.add_argument("--pilot", action="store_true", help="Lancer en mode pilote sur 100 offres.")
    args = parser.parse_args()

    run_id = args.run_id or f"triage_{int(time.time())}"
    output_dir = PROJECT_ROOT / "data" / "processed" / "matching" / "free_work_vs_france_travail" / run_id
    ...
    if checkpoint_file.exists() and triage_results_file.exists():
        try:
            ...
            if (checkpoint.get("free_work_hash") == fw_hash and 
                ...
                print(f"Resuming triage. Loaded {len(processed_offers)} completed offers from checkpoint.")
            else:
    ...
    # Category Counters
    counters = {"DUPLICATE_HIGH_CONFIDENCE": 0, "PROBABLY_NEW": 0, "HUMAN_REVIEW_REQUIRED": 0, "PROCESSING_ERROR": 0}
    ...
    for idx, fw_item in enumerate(fw_data):
        ...
        try:
            ...
            if fw_company_norm:
                ...
                for tok in extraire_tokens(fw_company_norm):
                    for fid in offers_by_company_token.get(tok, []):
                        ...
                        if ft_comp and SequenceMatcher(None, fw_company_norm, ft_comp).ratio() >= 0.8:
                            candidates_blocks_map[fid].add("COMPANY_MATCH")

            # RARE_TITLE_TOKENS
            title_tokens_with_freq = sorted([(t, doc_freq_title.get(t, 0)) for t in fw_tokens if 0 < doc_freq_title.get(t, 0) < MAX_RARE_TOKEN_DOCUMENT_FREQUENCY], key=lambda x: (x[1], x[0]))
            ...
            for fid in candidates_blocks_map:
                ...
                title_weighted = sum(idf_title.get(t, 1.0) for t in (t1 & t2)) / sum(idf_title.get(t, 1.0) for t in t1) if t1 else 0
                title_compact_seq = SequenceMatcher(None, fw_title_compact, ft_item["title_compact"]).ratio() if fw_title_compact or ft_item["title_compact"] else 0
                
                title_score = 45 * (title_seq * 0.25 + title_jac * 0.25 + title_weighted * 0.25 + title_compact_seq * 0.25)
                ...
                if fw_desc_norm and ft_item["desc_norm"]:
                    ...
                    desc_weighted = sum(idf_desc.get(t, 1.0) for t in (d1 & d2)) / sum(idf_desc.get(t, 1.0) for t in d1) if d1 else 0
                ...
                if fw_company_norm and ft_item["company_norm"]:
                    comp_seq = SequenceMatcher(None, fw_company_norm, ft_item["company_norm"]).ratio()
                ...
                coverage = 45 + (25 if fw_desc_norm and ft_item["desc_norm"] else 0) + (15 if fw_pc and ft_item["postal_code"] else 0) + (10 if fw_company_norm and ft_item["company_norm"] else 0) + (5 if rome_match else 0)

                comp_res, comp_sim_val = match_entreprises(fw_company_norm, ft_item["company_norm"])
                geo_res, _ = match_geographie(fw_pc, fw_locality_norm, fw_dept, ft_item["postal_code"], ft_item["locality_norm"], ft_item["department"])
            ...
            scored_candidates.sort(key=lambda x: (-x["preliminary_match_score"], x["france_travail_id"]))
        ...
        if (idx + 1) % 25 == 0 or (now - last_save_time) >= 5.0 or (idx + 1) == total_offers:
            ...
            print(f"Doublons probables : {counters['DUPLICATE_HIGH_CONFIDENCE']} | Nouvelles : {counters['PROBABLY_NEW']} | Revue humaine : {counters['HUMAN_REVIEW_REQUIRED']} | Erreurs : {counters['PROCESSING_ERROR']}")
            ...
            ecrire_progres_benchmark(PROJECT_ROOT / "data" / "processed" / "matching", progress_data)

            # Atomic save of incremental results
            ecriture_atomique(candidate_matches_file, json.dumps(results_candidate_matches, ensure_ascii=False, indent=2).encode("utf-8") + b"\n")
            ecriture_atomique(triage_results_file, json.dumps(results_triage_results, ensure_ascii=False, indent=2).encode("utf-8") + b"\n")
            ...
            ecriture_atomique(checkpoint_file, json.dumps(manifest, ensure_ascii=False, indent=2).encode("utf-8") + b"\n")

    # [3/4] Save final categorised lists and queue files
    print("Writing final triage output files...")

    # Filter category outputs
    duplicates = [x for x in results_triage_results if x["triage_category"] == "DUPLICATE_HIGH_CONFIDENCE"]
    probably_new = [x for x in results_triage_results if x["triage_category"] == "PROBABLY_NEW"]
    human_review = [x for x in results_triage_results if x["triage_category"] == "HUMAN_REVIEW_REQUIRED"]
    errors = [x for x in results_triage_results if x["triage_category"] == "PROCESSING_ERROR"]

    # Write duplicates
    ecriture_atomique(output_dir / "duplicates_high_confidence.json", json.dumps(duplicates, ensure_ascii=False, indent=2).encode("utf-8") + b"\n")
    # Write probably new
    ecriture_atomique(output_dir / "probably_new.json", json.dumps(probably_new, ensure_ascii=False, indent=2).encode("utf-8") + b"\n")
    # Write errors
    ecriture_atomique(output_dir / "processing_errors.json", json.dumps(errors, ensure_ascii=False, indent=2).encode("utf-8") + b"\n")
    ...
    ecriture_atomique(output_dir / "probably_new_import_candidates.json", json.dumps(import_candidates, ensure_ascii=False, indent=2).encode("utf-8") + b"\n")
    ...
    ecriture_atomique(output_dir / "human_review_queue.json", json.dumps(review_queue, ensure_ascii=False, indent=2).encode("utf-8") + b"\n")
    ...
    try:
        ...
        with csv_path.open("w", newline="", encoding="utf-8") as csvfile:
            ...
            writer.writerow([
                "free_work_source_id", "free_work_title", "free_work_company", "free_work_location", "free_work_source_url",
                "best_france_travail_id", "best_match_score", "candidate_count",
                "human_decision", "human_selected_france_travail_id", "human_comment", "reviewed_at"
            ])
            for q in review_queue:
                ...
                writer.writerow([
                    q["free_work_source_id"], q["free_work_title"], q["free_work_company"], q["free_work_location"], q["free_work_source_url"],
                    best_ft_id, best_score, len(q["candidates_france_travail"]),
                    "", "", "", ""
                ])
    except Exception as e:
    ...
    ecriture_atomique(output_dir / "triage_summary.json", json.dumps(summary, ensure_ascii=False, indent=2).encode("utf-8") + b"\n")
    ...
    ecriture_atomique(checkpoint_file, json.dumps(final_manifest, ensure_ascii=False, indent=2).encode("utf-8") + b"\n")
