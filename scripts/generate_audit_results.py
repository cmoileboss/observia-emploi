...
def generate_explanation(category: str, reason_codes: list, best_cand: dict) -> str:
    if category == "DUPLICATE_HIGH_CONFIDENCE":
        if "EXACT_FINGERPRINT" in reason_codes:
            return "Doublon à forte confiance détecté par concordance exacte d'une empreinte (soit Titre + Entreprise + Code Postal, soit Titre + Code Postal, soit Titre + Entreprise, soit Titre + Description)."
        return "Titre presque identique, entreprise compatible et même code postal. La description présente également une forte similarité."
    elif category == "PROBABLY_NEW":
        if not best_cand:
            return "Aucun candidat France Travail crédible n’a été identifié pour considérer l’offre comme un doublon probable."
        score = best_cand.get("preliminary_match_score", 0)
        return f"Aucun candidat France Travail suffisamment crédible n’a été identifié pour considérer l’offre comme un doublon probable. Le meilleur résultat possède un score de {score:.2f}."
    elif category == "HUMAN_REVIEW_REQUIRED":
        reasons_str = ", ".join(reason_codes)
        if "AMBIGUOUS_SCORE_MARGIN" in reason_codes:
            return "Plusieurs candidats possèdent des scores de similarité très proches. Une vérification humaine est nécessaire pour trancher."
        if "COMPANY_MATCH_GEO_DIFFERS" in reason_codes:
            return "L'entreprise correspond, mais la localisation géographique est différente. Une vérification humaine est nécessaire."
        if "COMPANY_DIFFERS_STRONG_CONTENT" in reason_codes:
            return "Le titre et la description sont très similaires mais l'entreprise diffère. Une vérification humaine est nécessaire."
        return "Le titre et la description sont proches, mais l’entreprise et la localisation diffèrent. Une vérification humaine est nécessaire."
    return "Erreur lors du traitement de l'offre."

def main():
    run_id = "run_triage_full_20260624"
    run_dir = PROJECT_ROOT / "data" / "processed" / "matching" / "free_work_vs_france_travail" / run_id
    
    fw_input_path = PROJECT_ROOT / "data" / "processed" / "free_work" / "full_catalog" / "20260624_081715" / "offers_normalized.json"
    ft_input_path = PROJECT_ROOT / "data" / "processed" / "france_travail" / "snapshots" / "current" / "france_travail_offers_snapshot.json"
    ...
    if len(fw_data) != expected_fw_total:
        print(f"Error: Free-Work offers count is {len(fw_data)} but expected {expected_fw_total}.", file=sys.stderr)
        sys.exit(1)
    ...
    for fw_sid, fw_item in fw_by_id.items():
        ...
        free_work_info = {
            ...
            "title_raw": fw_title,
            "title_normalized": cand_item.get("free_work_title_normalized", ""),
            "company_raw": fw_item.get("company_name"),
            "company_normalized": cand_item.get("free_work_company_normalized", ""),
            "location_raw": fw_loc.get("locality") if isinstance(fw_loc, dict) else str(fw_loc),
            "locality_normalized": cand_item.get("free_work_location", {}).get("locality_normalized", ""),
            "postal_code": cand_item.get("free_work_location", {}).get("postal_code", ""),
            "department_code": cand_item.get("free_work_location", {}).get("department_code", ""),
        ...
        # Source trace
        source_trace = {
            "free_work_input_file": "data/processed/free_work/full_catalog/20260624_081715/offers_normalized.json",
            "free_work_source_id": fw_sid,
            "france_travail_input_file": "data/processed/france_travail/snapshots/current/france_travail_offers_snapshot.json",
            "france_travail_source_id": best_ft_candidate["source_id"] if best_ft_candidate else None,
    ...
    for cat, val in expected_counters.items():
        if counters[cat] != val:
            print(f"Error: Inconsistent counters for category {cat}. Expected {val}, got {counters[cat]}", file=sys.stderr)
            sys.exit(1)
    ...
    for item in hr_subset:
        ...
        if best_cand:
            ...
            title_comp_seq = best_cand["score_breakdown"]["title"]
    ...
    prioritized_queue.sort(key=lambda x: (
        ...
        -x["best_france_travail_candidate"]["score_total"] if x["best_france_travail_candidate"] else 0.0,
    ...
    with pq_csv_path.open("w", newline="", encoding="utf-8") as csvfile:
        ...
        writer.writerow([
            ...
            "best_france_travail_id", "best_match_score", "priority", "candidate_count",
            "human_decision", "human_selected_france_travail_id", "human_comment", "reviewed_at"
        ])
        for q in prioritized_queue:
            best_ft_id = q["best_france_travail_candidate"]["source_id"] if q["best_france_travail_candidate"] else ""
            best_score = q["best_france_travail_candidate"]["score_total"] if q["best_france_travail_candidate"] else ""
            cand_count = len(q["alternative_candidates"]) + (1 if q["best_france_travail_candidate"] else 0)
            writer.writerow([
    ...
    news.sort(key=lambda x: (x["best_france_travail_candidate"]["score_total"] if x["best_france_travail_candidate"] else 0.0, x["free_work"]["source_id"]))
    ...
    case_sap = triage_by_id.get("513371")
    # Wait, the sap ID might be different or not. Let's look up by title/company/location or check by ID.
    # Let's print out what categories they got.
    
    def check_and_print_case(source_id, name):
        item = triage_by_id.get(source_id)
        if item:
            print(f"Historical case {name} ({source_id}): Category = {item['triage_category']}, reason_codes = {item['triage_reason_codes']}")
        else:
