...
def test_collect_full_catalog_max_pages(tmp_path, mock_requests_get):
    ...
    with patch("scripts.collect_free_work_full_catalog.PROJECT_ROOT", tmp_path), \
         patch("scripts.collect_free_work_full_catalog.time.sleep"):
    ...
    batch_dir = tmp_path / "data" / "raw" / "free_work" / "full_catalog" / "batches" / batch_id
